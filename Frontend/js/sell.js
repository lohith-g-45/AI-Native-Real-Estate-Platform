...
function renderListings(groupedListings) {
  ...
  if (allListings.length === 0) {
    ...
    // Aggregate totals for the hero dashboard
    let totalViews = 0, totalSaves = 0, totalInquiries = 0, totalOffers = 0;

    allListings.forEach(listing => {
      if (listing.analytics) {
        totalViews += (listing.analytics.total_views || 0);
        totalSaves += (listing.analytics.total_saves || 0);
        totalInquiries += (listing.analytics.total_inquiries || 0);
        totalOffers += (listing.analytics.total_offers || 0);
      }
    });

    // Update hero dashboard stats if the elements exist
    const elViews = document.getElementById('stat-total-views');
    const elSaves = document.getElementById('stat-total-saves');
    const elInquiries = document.getElementById('stat-total-inquiries');
    const elOffers = document.getElementById('stat-total-offers');
    if (elViews) elViews.textContent = totalViews.toLocaleString();
    if (elSaves) elSaves.textContent = totalSaves.toLocaleString();
    if (elInquiries) elInquiries.textContent = totalInquiries.toLocaleString();
    if (elOffers) elOffers.textContent = totalOffers.toLocaleString();
    ...
    allListings.forEach(listing => {
      ...
      // Map analytics data
      const views = listing.analytics?.total_views || 0;
      const saves = listing.analytics?.total_saves || 0;
      const inquiries = listing.analytics?.total_inquiries || 0;
          ...
          <div style="display:flex; justify-content:space-between; align-items:flex-start;">
            <div>
              ...
              <div style="display:flex; gap:24px; color:#475569; font-size:13px; font-weight:600; margin-bottom:24px;">
                <span style="display:flex; align-items:center; gap:6px;"><i data-feather="eye" style="width:16px;height:16px;"></i> ${views.toLocaleString()}</span>
                <span style="display:flex; align-items:center; gap:6px;"><i data-feather="heart" style="width:16px;height:16px;"></i> ${saves.toLocaleString()}</span>
                <span style="display:flex; align-items:center; gap:6px;"><i data-feather="message-square" style="width:16px;height:16px;"></i> ${inquiries.toLocaleString()}</span>
              </div>
...
async function fetchSellerProfile() {
  ...
  try {
    const isLocal = ['localhost', '127.0.0.1'].includes(window.location.hostname) || window.location.hostname.startsWith('192.168.') || window.location.hostname.startsWith('10.');
    const AUTH_BASE = isLocal ? `http://${window.location.hostname}:3000/v1/auth` : 'https://ai-native-real-estate-platform.onrender.com/v1/auth';
    ...
    if (res.ok) {
      ...
      const name = data.fullName || email.split('@')[0] || 'Seller';
      const el = document.getElementById('seller-greeting-name');
      if (el) el.textContent = name;
    }
  } catch(e) {
    console.error('Failed to load profile for dashboard header', e);
  }
}
